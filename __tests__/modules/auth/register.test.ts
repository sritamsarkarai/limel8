...
describe("registerUser", () => {
  ...
  it("creates user and profile in a transaction on success", async () => {
    ...
    const result = await registerUser({ email: "a@b.com", password: "pass", name: "Test" });
  ...
  it("hashes the password before storing", async () => {
    ...
    (db.$transaction as jest.Mock).mockImplementation(async (fn: any) => {
      const tx = {
        user: { create: jest.fn().mockImplementation(async ({ data }: any) => { capturedHash = data.passwordHash; return { id: "u1", email: "a@b.com" }; }) },

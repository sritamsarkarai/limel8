...
export default async function MarketplacePage({
  ...
  searchParams: Promise<{ cursor?: string; q?: string; location?: string }>;
}) {
  const { cursor, q, location } = await searchParams;
  const listings = await getListings({ cursor, q, location });
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8">
      ...
      <MarketplaceSearch defaultQ={q ?? ""} defaultLocation={location ?? ""} />
      ...
      {listings.length === 0 ? (
        <div className="py-16 text-center relative">
          ...
          <p className="relative text-zinc-400">
            {q || location ? "No listings match your search." : "No listings yet. Be the first to list something!"}
          </p>
        </div>
      ) : (
      ...
      {listings.length === 24 && (
        <div className="mt-8 flex justify-center">
          <Link
            href={`/marketplace?cursor=${listings[listings.length - 1].id}${q ? `&q=${encodeURIComponent(q)}` : ""}${location ? `&location=${encodeURIComponent(location)}` : ""}`}
